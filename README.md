...
- 入力値のリセットボタン（スコア・チップ各エリア）
- 入力中の値のプラス（緑）/マイナス（赤）色分け表示
...
├── generate_assets.py          # アプリアイコン・プロモーション画像生成スクリプト
├── assets/
...
### 共有データ形式（ShareGameData v2）

ゲーム共有時にBase64エンコードされるJSONの構造：

| キー | 型 | 説明 |
|------|-----|------|
| v | number | データバージョン（現在: 2） |
| pc | number | プレイヤー数（3または4） |
| d | string | ゲーム開始日（YYYY/MM/DD） |
| p | string[] | プレイヤー名の配列 |
| s | array[] | スコアデータ [半荘番号, プレイヤーインデックス, ポイント] |
| c | array[] | チップデータ [半荘番号, プレイヤーインデックス, チップポイント]（任意） |

※ v1形式（旧フォーマット）のインポートにも後方互換で対応
...
- 範囲: -200 〜 +200
- 初期値: 0
- プラス値は緑、マイナス値は赤で色分け表示
- 各入力エリアにリセットボタン（全プレイヤーの値を0に戻す）
